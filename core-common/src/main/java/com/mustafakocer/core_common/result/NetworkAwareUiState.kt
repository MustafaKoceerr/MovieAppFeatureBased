...
/**
 * Convert regular UiState to NetworkAwareUiState
 * Useful for gradual migration from existing code
 */
fun <T> UiState<T>.toNetworkAware(): NetworkAwareUiState<T> {
    return when (this) {
        is UiState.Idle -> NetworkAwareUiState.Idle
        is UiState.Loading -> NetworkAwareUiState.InitialLoading(message)
        is UiState.Success -> NetworkAwareUiState.Success(data)
        is UiState.Error -> NetworkAwareUiState.Error(exception)
    }
}

/**
 * Convert NetworkAwareUiState to regular UiState
 * Useful when interfacing with existing code that expects UiState
 */
fun <T> NetworkAwareUiState<T>.toRegularUiState(): UiState<T> {
    return when (this) {
        is NetworkAwareUiState.Idle -> UiState.Idle
        is NetworkAwareUiState.InitialLoading -> UiState.Loading(message)
        is NetworkAwareUiState.RefreshLoading -> UiState.Success(currentData)
        is NetworkAwareUiState.Success -> UiState.Success(data)
        is NetworkAwareUiState.SuccessWithNetworkError -> UiState.Success(data)
        is NetworkAwareUiState.Error -> UiState.Error(exception)
    }
}